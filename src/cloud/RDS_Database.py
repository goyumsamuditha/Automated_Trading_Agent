...
load_dotenv()
DB_URL=(f'mysql+pymysql://{os.getenv("RDS_User")}:{os.getenv("RDS_password")}@{os.getenv("RDS_HOST")}:3306/{os.getenv("RDS_DB",'database-1')}')  # construct the database URL
...
def push_asset_features(ticker):                         
    ...
    safe = df.replace('-','_')         # replace any hyphens in the DataFrame with underscores to ensure compatibility with SQL column names
    df = pd.read_csv(f'data/featured/(safe).csv', index_col = 'Date', parse_dates = True )  # read the CSV file containing the asset features into a DataFrame, using the 'Date' column as the index and parsing it as dates
    ...
    df.rename(columns={
        'Date':'date','Close':'close','RSI_14':'rsi_14',
        'MACD':'macd','SMA_20':'sma_20','SMA_50':'sma_50',
        'BB_Width':'bb_width','Volatility_20':'volatility',
        'Label':'label'}, inplace=True)  # rename the columns of the DataFrame to match the column names in the asset_features table in the database
    ...
    df[cols].to_sql('asset_features', engine,
